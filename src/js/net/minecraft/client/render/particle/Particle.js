...
export default class Particle extends Entity {
    ...
    rebuild() {
        ...
        let textureAtlas = this.minecraft.worldRenderer.textureAtlas;

        if (textureAtlas && textureAtlas.isLoaded() && this.textureName) {
            tessellator.bindTexture(textureAtlas.getTexture());

            let uvs = textureAtlas.getUVs(this.textureName);
            let tileWidth = uvs.maxU - uvs.minU;
            let tileHeight = uvs.maxV - uvs.minV;
            let snippetWidth = tileWidth / 4.0;
            let snippetHeight = tileHeight / 4.0;
            let offsetU = (this.randomX / 4.0) * (tileWidth - snippetWidth);
            let offsetV = (this.randomY / 4.0) * (tileHeight - snippetHeight);

            let minU = uvs.minU + offsetU;
            let maxU = minU + snippetWidth;
            let minV = 1.0 - (uvs.maxV - offsetV);
            let maxV = 1.0 - (uvs.minV + offsetV + snippetHeight);
