...
export default class BlockBluestoneLamp extends Block {
    ...
    updateState(world, x, y, z) {
        ...
        if (powered !== current) {
            ...
            world.updateLight(EnumSkyBlock.BLOCK, x, y, z, x, y, z);
        }
    }

    getTextureForFace(face, data, x, y, z, world) {
        if (!world || typeof x !== 'number') {
            return 'bs-logic:bluestoneLampOff';
        }
        return this.getState(world, x, y, z) === 1
            ? 'bs-logic:bluestoneLampOn'
            : 'bs-logic:bluestoneLampOff';
    }

    getLightValue(world, x, y, z) {
        if (!world || typeof x !== 'number') return 0;
        return this.getState(world, x, y, z) === 1 ? 15 : 0;
    }
